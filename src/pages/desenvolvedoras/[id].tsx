...
const FormAlterDesenvolvedoras = () => {
    ...
    useEffect(() => {
        ...
        if (query.id) {
            ...
            axios.get('/api/desenvolvedoras/' + query.id).then(resultado => {
                const desenvolvedor = resultado.data

                for (let atributo in desenvolvedor) {
                    setValue(atributo as 'nome' | 'pais' | 'fundacao' | 'logo', desenvolvedor[atributo])
                }
            })
        }
    ...
    function salvar(dados: any) {
        axios.put('/api/desenvolvedoras/' + dados.id, dados)
    ...
    return (
        ...
        <>
            ...
            <div className='container'>
                <Row>
                    <span className='text-5xl p-2 justify-center flex mb-5 rounded-full font-bold box-decoration-slice bg-gradient-to-r from-indigo-600 to-cyan-500 text-white px-3'>Editar Desenvolvedora</span>
                </Row>
